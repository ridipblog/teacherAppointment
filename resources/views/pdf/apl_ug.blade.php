...
<body>
    ...
    <p>
        ...
        time without notice and without assigning any reason thereof,
        Sri/Ms <span class="underline"> {{ $candDetails->name ?? '' }}</span> , S/o,/D/o,/W/o
        <span class="underline">{{ $candDetails->fatherName ?? '' }}</span>,
        Address: <span class="underline">{{ $candDetails->address ?? '' }}</span>, Dist:
        <span class="underline">{{ $candDetails->district ?? '' }}</span>
        is hereby appointed as
        <span class="underline">{{ $candDetails->vacency_details->school_vacency->vacencyCategory ?? 'Assistant Teacher' }}</span>
        (<span class="underline">{{ $candDetails->vacency_details->school_vacency->medium ?? '' }}</span>)
        in the scale of pay (PB-2) Rs. 14,000/- to Rs. 70,000/- and Grade pay Rs.
        <span class="underline">{{ $candDetails->gradePay ?? '7400' }}</span>/- p.m. plus other allowances as admissible
        under the Govt. rule and posted at
        <span class="underline">{{ $candDetails->vacency_details->school_vacency->schoolName ?? '' }}</span> vice
    ...
    <p>
        The said filled up post has the approval of Finance (SIU) Deptt’s vide ECF No.
        <span class="underline">{{$candDetails->vacency_details->ecfNO ?? '.................' }}</span>,
        Dated Dispur the <span class="underline">{{ $candDetails->financeApprovalDate ?? '14th August, 2025' }}</span>.
    </p>
    ...
    <p style="margin-left: 30px; font-style: italic;">
        ...
        01-02-2005 shall not be governed by the existing Assam Service (pension) rules, 1969 and order issued there under
        from time to time and that the pension and other retirement benefit will be by a set-up new pension rules which
        are being formulated in the line with the contributory pension scheme of the Govt. of India going to be notified
        in due course”.
    </p>
    ...
    <div class="footer">
        <p><strong>Memo No. DE/BTC/Aptt****/****<span
                    class="underline">{{ $candDetails->letterCode ?? '' }}</span>-A,</strong>
            Dated Kokrajhar the
