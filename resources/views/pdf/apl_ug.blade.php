...
<body>
    ...
    <body>
        ...
        <p>
            <strong>No. DE/BTC/Aptt****/****
                {{-- <span class="underline">{{ $candDetails->letterCode ?? '' }}</span> --}}
            </strong>.
            ...
            any time without notice and without assigning any reason thereof,
            Sri/Ms <span class="underline">{{ $candDetails->name ?? '' }}</span>, S/o, D/o, W/o <span
                class="underline">{{ $candDetails->fatherName ?? '' }}</span>,
            Address: <span class="underline">{{ $candDetails->address ?? '' }}</span>, Dist: <span
                class="underline">{{ $candDetails->district ?? '' }}</span> is hereby appointed as
            <span
                class="underline">{{ $candDetails->vacency_details->school_vacency->vacencyCategory ?? 'Assistant Teacher' }}</span>
            (<span class="underline">{{ $candDetails->vacency_details->school_vacency->medium ?? '' }}</span>)
            in the scale of pay (PB-2) Rs. 14,000/- to Rs. 70,000/- and Grade pay Rs. 7400/- p.m. plus other allowances
            as admissible under the Govt. rule and posted at
            ...
            Retired/Expired/VRS/Transferred/Promoted. {{-- Verible name Required  --}}
        </p>
