...
<head>
    ...
    <style>
      ...
      @page {
        size: legal portrait; /* page size */
        margin: 10mm;        /* remove default browser margins */
      }
    }
    @media print {
  header,
...
<body>
    ...
    <p>
        ...
        In exercise of power conferred upon the Director of Education, BTC, Kokrajhar by the Secretary, BTC, Kokrajhar
        vide letter No. BTC/Edn(El)-35/2005/1, dated, 13th Sept/2005 and in pursuance of approval of BTC vide letter
        No. <span class="underline">{{ $candDetails->letterCode ?? '' }}</span>, dtd
        <span class="underline">{{ $candDetails->generatedOn ?? '..........' }}</span> and subject to discharge at any
        time without notice and without assigning any reason thereof, Sri/Ms <span
            ...
            class="underline">{{ $candDetails->district ?? '' }}</span> is hereby appointed as <span
            class="underline">{{ $candDetails->vacency_details->school_vacency->vacencyCategory ?? '' }}</span>
        (<span class="underline">{{ $candDetails->vacency_details->school_vacency->medium ?? '' }}</span>) in the scale
        of pay (PB-2) Rs. 14,000/- to Rs. 70,000/- and Grade pay Rs.
        <span class="underline">{{ $candDetails->gradePay ?? '8700' }}</span>/- p.m. plus other allowances as admissible
        under the Govt. rule and posted at <span
            class="underline">{{ $candDetails->vacency_details->school_vacency->schoolName ?? '' }}</span> vice <span
            class="underline">{{ $candDetails->vacency_details->replcedPersion ?? '' }}</span>,
            <span
            class="underline">{{ $candDetails->vacency_details->replaceReason ?? 'Retired/Expired/VRS/Transferred/Promoted' }}.</span>,
    </p>

    <p>
        The said filled up post has the approval of Finance (SIU) Deptt’s vide ECF No. <span
            class="underline">{{ $candDetails->vacency_details->ecfNO ?? '.................' }}</span>,
        Dated Dispur the 14th August, 2025.
    </p>
    ...
    <p style="margin-left: 30px; font-style: italic;">
        “I understand and accept that the Govt. Servant joining the service in the State Govt. on or after 01-02-2005
        shall not be governed by the existing Assam Service (pension) rules, 1969 and order issued there under from time
        to time and that the pension and other retirement benefit will be by a set-up new pension rules which are being
        formulated in the line with the contributory pension scheme of the Govt. of India going to be notified in due
        course”.
    </p>
    ...
    <br><br><br>
    <button onclick="window.print()" id="printButton">Print</button>
